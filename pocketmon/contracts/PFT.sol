...
contract PFT is KIP17Full, Ownable {
    ...
    uint256 public constant pocketmonPrice = 2000000000000000000; // 2 Klay
    uint256 public constant maxPocketmon = 257;
    uint public constant maxPocketmonPurchase = 4;

    constructor() public KIP17Full("Pocketmon NFT", "PFT") {}
    ...
    function mintNFT(string memory tokenURI)
        ...
        returns (uint256)
    {
        require(msg.value == pocketmonPrice, "Klay value sent is not correct");
        _tokenIds.increment();
        require(totalSupply().add(1) <= maxPocketmon, "Minting would exceed max supply of pocketmon");
        require(balanceOf(msg.sender)<= maxPocketmonPurchase, "Can only mint 4 tokens per person");
